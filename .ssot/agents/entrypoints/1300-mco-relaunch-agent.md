...
Force relaunch a specific agent for a project's latest cycle using the MCPCO `force_relaunch_agent` tool.

## MCPCO tool

`force_relaunch_agent(project: str, agent: int, orientation_preamble: str | None = None) -> dict`

- `project` — the project name (resolved via project aliases).
- `agent` — the agent number to relaunch; validated against `(1, 2, 3, 4)` (`mcp_server.py:865-868`).
- `orientation_preamble` — optional preamble text passed to the launched session.

The tool is annotated `MUTATING` (`readOnlyHint=False, destructiveHint=True`, `mcp_server.py:38,853`).
...
Call the MCPCO MCP tool `force_relaunch_agent` directly. The client's own annotation-driven approval
...
Go through the numbered binding `1300-mco-relaunch-agent` in `entrypoint-bindings.mjs`, which dispatches
`mco-adapter.mjs --operation invoke --capability force-relaunch-agent`. The adapter requires
